...
function getRevenueSummary({ publisher, month }) {
  ...
  return JSON.stringify({
    ...
    monthly_data: months.map(mo => ({
      ...
      impressions: data.impressions[mo], cpm: data.cpm[mo], fill_rate: data.fill_rate[mo],
    })),
  });
}

function getUnderdelivering({ threshold = 70 }) {
  const list = DUMMY_DATA.campaigns.filter(c => c.status === "active" && c.delivery_pct < threshold);
...
function getPublisherPerformance({ publisher }) {
  ...
  return JSON.stringify({
    publisher: data.name, revenue: data.revenue, fill_rates: data.fill_rate,
    top_segments: data.top_segments, inventory,
    active_campaigns: campaigns.filter(c => c.status === "active").length, campaigns,
  });
}
